...
const iconMap = {
  ...
  'Smartphones y Accesorios': (
    <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
  ),
  'Ropa': (
  ...
  'Videojuegos y Consolas': (
    <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M6.144 7.5a4.5 4.5 0 014.05-3h3.612a4.5 4.5 0 014.05 3.03l2.4 6a4.5 4.5 0 01-6.547 5.22L12 19.5l-1.749.75a4.5 4.5 0 01-6.547-5.22l2.44-6.03zm6.606 6.75h3m-1.5 1.5v-3" /></svg>
  ),
  'Espacios': (
  ...
  'Otros': (
    <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
  ),
}

const defaultIcon = (
  <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
)

function CategoryGrid({ categories, selected = [], onToggle }) {
  const list = categories?.length > 0 ? categories : CATEGORIES
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8">
          {list.map((cat) => {
            ...
            return (
              ...
              >
                <span className="text-cyan-300 [&>svg]:h-6 [&>svg]:w-6 sm:[&>svg]:h-8 sm:[&>svg]:w-8">{iconMap[cat] || defaultIcon}</span>
